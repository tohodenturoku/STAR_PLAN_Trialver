Shader "Custom/FlameWithStrongGradient"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                // **はっきりとしたグラデーション（白 → 黄 → オレンジ）**
                // 白色部分を80%の割合にして、下端で黄色、オレンジ色に変化
                float gradient = smoothstep(0.0, 0.8, i.uv.y + 0.5);  // 0.8で白色の割合を80%に
                half4 flameColor = lerp(_Color1, _Color2, gradient * 1.0); // 白 → 黄
                flameColor = lerp(flameColor, _Color3, gradient * 1.5); // 黄 → オレンジ

                // **下端部分で模様がくっつかないように透明度調整**
                if (i.uv.y < -0.5) // 下端が透明になるように調整
                {
                    flameColor.a = 0.0;
                }

                // **先端のボカし処理**（オレンジ色部分をぼかす）
